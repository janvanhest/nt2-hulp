...
2. Een invulzin heeft minimaal `sentence_template` en `answer` (zie `v3/erd.md`); de beheerder geeft aan welke werkwoordsvorm het antwoord is (bijv. ik-tt, hij-vt), zodat eenduidig is of "loop" of "liep" e.d. bedoeld is.
...
- **ERD:** `INVULZIN` (o.a. `werkwoord_id`, `sentence_template`, `answer`, `answer_form_key`) en relatie `WERKWOORD ||--o{ INVULZIN` in `v3/erd.md`.
